...
alter table if exists event add constraint FKs0d4oegtq5sif4e0eoxxt9u5i foreign key (recipe_id) references recipes

alter table if exists recipes add constraint FK562qxrf6asv465kol8skqtgx8 foreign key (created_by) references users
...
alter table if exists users_roles add constraint FK2o0jvgh89lemvvo17cbqvdxaa foreign key (user_id) references users
    ...
    values ('1', '2023-10-14 19:54:49.522580', 'https://o-tendencii.com/uploads/posts/2022-03/1646158220_40-o-tendencii-com-p-appetitnie-blini-foto-45.jpg', 'Молоко - 0,5 л.
Яйца - 3 шт.
...
3. Хорошо разогреть сковороду, слегка смазать ее топленым маслом и налить на нее столько теста, сколько нужно, чтобы получить тонкий блинчик. Жарить по 1–2 минуты с каждой стороны.', 'Блины традиционные', '2023-10-14 19:54:49.522580')

    insert into recipes (created_by, created_date_time, photo_url, recipe_content, recipe_title, update_date_time)
    values ('1', '2023-10-14 20:04:41.589534', 'https://thebrilliantkitchen.com/wp-content/uploads/2022/08/Bloody-Mary.jpg', 'Водка - 250 мл.
Томатный сок - 1 л.
...
3. Украсить кусочками лимона, в каждый бокал положить по палочке сельдерея (для помешивания) и подавать.', 'Коктейль «Кровавая Мэри»', '2023-10-14 20:04:41.589534')

    insert into recipes (created_by, created_date_time, photo_url, recipe_content, recipe_title, update_date_time)
    values ('1', '2023-10-14 20:07:21.833000', 'https://mir-s3-cdn-cf.behance.net/project_modules/fs/a164bf91539421.5e34324a88257.jpg', 'Молоко - 300 мл.
...
Разрезать слоеное дрожжевое тесто на две части. Половину теста раскатать в пласт толщиной 7-8 мм (вторую половину теста завернуть в плёнку и поместить в холодильник, или заморозить впрок). Вырезать из теста треугольники с боковыми сторонами 20 см и основанием около 18 см (получилось шесть штук). Точно так же поступить со второй половиной теста.', 'Круассан ', '2023-10-14 20:14:59.785689')

    insert into recipes (created_by, created_date_time, photo_url, recipe_content, recipe_title, update_date_time)
    values ('1', '2023-10-14 20:22:08.636814', 'https://www.zipangprovisions.com/wp-content/uploads/2021/02/Health-advantages-of-consuming-cakes.jpg', 'Пшеничная мука - 530 г.
Сахар - 300 г.
Какао-порошок - 300 г.
Сода - 1 чайная ложка.
...
6. Собрать торт, смазав коржи кремом. Можно есть его сразу, но лучше оставить часа на три в холодильнике.', 'Шоколадный торт', '2023-10-14 20:22:08.636814')

    insert into event (date, location, name, photo_url, recipe_id, tradition)
    values ('56 дней от даты Пасхи', 'Россия', 'Масленица', 'https://volontermo.ru/b/c/25870.jpg', '1', 'Масленица — один из самых известных старинных народных праздников, который продолжают отмечать до сих пор. Торжество длится целую неделю и предваряет Великий пост: в эти дни пекут блины и сжигают чучело, чтобы проводить зиму и встретить весну. «Лента.ру» рассказывает, когда масленичная неделя начнется в 2024 году, а также вспоминает историю и традиции праздника. Масленица отмечается в течение недели перед Великим постом. Сразу после масленичной недели наступает Чистый понедельник, который в 2024 году выпадает на 18 марта. Это начало Великого поста, который продлится 40 дней.')

    insert into event (date, location, name, photo_url, recipe_id, tradition)
    values ('каждый год 30 января', 'США', 'День круасана', 'https://lp-cms-production.imgix.net/2022-07/GettyRF_1070622360.jpg?auto=format&q=75&w=1920', '3', 'В Соединенных Штатах Национальный День Круассана отмечается каждый год 30 января. Круассаны - это масляные рулетики в форме полумесяца, которые хрустящие снаружи и мягкие внутри.')

    insert into event (date, location, name, photo_url, recipe_id, tradition)
    values ('каждый год 27 января', 'США', 'День шоколадного торта', 'https://zbuzz.de/wp-content/uploads/2021/11/Lustige-Geburtstagssprüche-für-Frauen.jpeg', '4', 'Национальный день шоколадного торта - неофициальный праздник, который отмечается 27 января каждого года. Этот день посвящен этому сладкому удовольствию, которое было частью американского общества около 252 лет.')
